...
void LabelDialog::setupUI()
{
    ...
    // Validators
    _addressEdit->setPlaceholderText(tr("0x1234, $1234, #1234 or decimal"));
    _addressEdit->setToolTip(tr("Z80 address (0x1234, $1234, #1234 or decimal)"));
...
void LabelDialog::loadLabelData()
{
    ...
    _bankAddressEdit->setText(
        _label.bankOffset == UINT16_MAX ? "" : QString("%1").arg(_label.bankOffset, 4, 16, QChar('0')).toUpper());
...
void LabelDialog::applyUIDataToLabel()
{
    ...
    // Bank - support hex with 0x, $, # prefixes or decimal
    ...
    else
    {
        ...
        // Try hex if has prefix or hex chars
        if (bankStr.startsWith("0X") || bankStr.startsWith("$") || bankStr.startsWith("#") ||
            bankStr.contains(QRegularExpression("[A-F]")))
        {
            // Remove prefixes
            if (bankStr.startsWith("0X"))
                bankStr = bankStr.mid(2);
            else if (bankStr.startsWith("$") || bankStr.startsWith("#"))
                bankStr = bankStr.mid(1);

            if (!bankStr.isEmpty())
            {
                bank = bankStr.toUShort(&ok, 16);
            }
        }
        // Try decimal
        if (!ok)
        {
            bank = bankStr.toUShort(&ok, 10);
        }

        if (ok && bank < 256)
        {
            // 256 is MAX_RAM_PAGES
            _label.bank = bank;
        }
        else
        {
    ...
    else
    {
        ...
        if (!ok)
            _label.bankOffset = UINT16_MAX;  // Default to any offset on error
    }
...
void LabelDialog::validateInput()
{
    ...
    // This requires access to LabelManager: if (_labelManager && _mode == Mode::Add &&
    // _labelManager->GetLabelByName(...)) ...
    ...
    else if (!_bankAddressEdit->text().trimmed().isEmpty() && bankVal == 0xFF)
    // Bank not specified, but bank address is given
    {
...
bool LabelDialog::isAddressValid(const QString& addressStr, uint16_t& addressValue)
{
    ...
    // If empty after removing prefix
    if (cleanStr.isEmpty())
    {
        return false;
    }
    ...
    const QRegularExpression hexRegex("^[0-9A-Fa-f]+$");
    if (isHex || hexRegex.match(cleanStr).hasMatch())
    {
        bool ok;
        uint value = cleanStr.toUInt(&ok, 16);
        if (ok && value <= 0xFFFF)
        {
            addressValue = static_cast<uint16_t>(value);
            return true;
        }
    }

    // Try decimal parsing if hex parsing failed or no hex indicator
    bool ok;
    uint value = cleanStr.toUInt(&ok, 10);
    if (ok && value <= 0xFFFF)
    {
        addressValue = static_cast<uint16_t>(value);
        return true;
...
void LabelDialog::onAccept()
{
    ...
    if (_mode == Mode::Add)
    {
        ...
        {
            QMessageBox::warning(
                this, tr("Add Label Failed"),
                tr("A label with the name '%1' already exists.").arg(QString::fromStdString(_label.name)));
            return;
        }
    }
