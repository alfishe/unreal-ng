...
BreakpointDialog::BreakpointDialog(Emulator* emulator, QWidget* parent)
    : QDialog(parent), _emulator(emulator)
{
    ...
    Qt::SortOrder sortOrder = static_cast<Qt::SortOrder>(
        settings.value("BreakpointDialog/sortOrder", Qt::AscendingOrder).toInt());
...
BreakpointDialog::~BreakpointDialog()
{
    ...
    for (int i = 0; i < _breakpointTable->columnCount(); ++i)
    {
        settings.setValue(QString("BreakpointDialog/column%1Width").arg(i), 
                         _breakpointTable->columnWidth(i));
    }
    
    // Save sort column and order
    settings.setValue("BreakpointDialog/sortColumn", 
                     _breakpointTable->horizontalHeader()->sortIndicatorSection());
    settings.setValue("BreakpointDialog/sortOrder", 
                     _breakpointTable->horizontalHeader()->sortIndicatorOrder());
}
...
void BreakpointDialog::setupUI()
{
    ...
    _breakpointTable = new QTableWidget(this);
    ...
    // Connect signals
    connect(_breakpointTable, &QTableWidget::itemSelectionChanged, this, &BreakpointDialog::onBreakpointSelectionChanged);
    ...
    connect(_enableButton, &QPushButton::clicked, [this]() {
        toggleBreakpoint();
        _breakpointTable->selectedItems().at(0)->tableWidget()->item(
            _breakpointTable->selectedItems().at(0)->row(), 4)->setText("Active");
    });
    connect(_disableButton, &QPushButton::clicked, [this]() {
        toggleBreakpoint();
        _breakpointTable->selectedItems().at(0)->tableWidget()->item(
            _breakpointTable->selectedItems().at(0)->row(), 4)->setText("Inactive");
    });
...
void BreakpointDialog::setupGroupFilter()
{
    ...
    connect(_groupFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), 
            this, &BreakpointDialog::applyFilters);
...
void BreakpointDialog::setupTableContextMenu()
{
    ...
    connect(_breakpointTable, &QTableWidget::customContextMenuRequested, 
            this, &BreakpointDialog::showContextMenu);
}
...
void BreakpointDialog::populateBreakpointTable()
{
    ...
    for (const auto& pair : breakpoints)
    {
        ...
        switch (bp->type)
        {
            case BRK_MEMORY: typeStr = "Memory"; break;
            case BRK_IO: typeStr = "Port"; break;
            case BRK_KEYBOARD: typeStr = "Keyboard"; break;
        ...
        {
            if (bp->memoryType & BRK_MEM_READ) accessStr += "R";
            if (bp->memoryType & BRK_MEM_WRITE) accessStr += "W";
            if (bp->memoryType & BRK_MEM_EXECUTE) accessStr += "X";
        }
        else if (bp->type == BRK_IO)
        {
            if (bp->ioType & BRK_IO_IN) accessStr += "I";
            if (bp->ioType & BRK_IO_OUT) accessStr += "O";
        }
