...
void QHexView::reset()
{
    setDocument(QHexDocument::create(this));
}
...
void QHexView::gotoOffset(quint64 offset) const
{
    // Ensure that the offset is within the document bounds
    quint64 jumpOffset = offset;
    quint64 dataLength = (quint64)m_hexdocument->length();
    if (jumpOffset > dataLength)
    {
        jumpOffset = dataLength - 1;
    }
    ...
    uint8_t lineWidth = options().linelength;
    newPosition.line = jumpOffset / lineWidth;
    newPosition.column = jumpOffset % lineWidth;
    m_hexcursor->move(newPosition);
}
