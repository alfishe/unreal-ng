...
DisassemblerTextView::DisassemblerTextView(QWidget *parent) : QAbstractScrollArea(parent), m_disassembler(nullptr), m_disassemblerpopup(nullptr), m_actions(nullptr)
{
    ...
    QString logMessage = "Setting refresh rate to " + QString::number(refreshfreq, 'f', 1) + "Hz";
    m_refreshrate = static_cast<int>(std::ceil((1.0 / refreshfreq) * 1000));
...
size_t DisassemblerTextView::visibleLines() const
{
    ...
    size_t vl = static_cast<size_t>(std::ceil(this->height() / fm.height()));
...
size_t DisassemblerTextView::firstVisibleLine() const { return verticalScrollBar()->value(); }
size_t DisassemblerTextView::lastVisibleLine() const { return firstVisibleLine() + visibleLines() - 1; }
...
void DisassemblerTextView::mousePressEvent(QMouseEvent *e)
{
    ...
    {
        // currentDocument()->cursor()->goBack();
    }
    else if (e->button() == Qt::ForwardButton)
    {
        // currentDocument()->cursor()->goForward();
    }
...
void DisassemblerTextView::paintLines(QPainter *painter, int first, int last)
{
...
bool DisassemblerTextView::isColumnVisible(size_t column, size_t *xpos)
{
    ...
    int adv = fontMetrics().horizontalAdvance(" ");
#else
    int adv = fontMetrics().width(" ");
#endif
...
QRect DisassemblerTextView::lineRect(size_t line)
{
    ...
    int offset = line - firstVisibleLine();
