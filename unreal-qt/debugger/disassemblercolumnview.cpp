...
void DisassemblerColumnView::setDisassembler(Z80Disassembler* disassembler)
{
...
void DisassemblerColumnView::paintEvent(QPaintEvent*)
{
    ...
    int w = fm.horizontalAdvance(" "), h = fm.height(), x = this->width() - (w * 2);
...
bool DisassemblerColumnView::isPathSelected(const DisassemblerColumnView::ArrowPath &path) const
{
    ...
    //auto& document = m_disassembler->document();
    unsigned line = 0; //document->cursor()->currentLine();
    result = (line == path.startidx) || (line == path.endidx);
...
{
    int w = fm.horizontalAdvance(" ") / 2, hl = fm.height() / 3;
