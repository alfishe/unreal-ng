...
struct UiStateSeed {
    ...
    // TSConf fork UI state (TDD-DBG-02 §3/§5.3)
...
struct TestSetState {
    ...
    UiStateSeed ui;
    // TSConf fork data (TDD-DBG-02): the golden sample runs the fork layout
    // with ROM 0 in window 0 and the ts register block from the oracle.
    bool isTsConf = false;
    TsConfState ts;
    std::vector<PcHistEntry> pcHist;  // newest first (<= 28 shown)
};
