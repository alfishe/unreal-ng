...
public:
    ...
    // -- TSConf fork additions (TDD-DBG-02 §5) --------------------------------------------
    // Ring of the last M1 PC fetches, newest first (§5.1 PC history panel).
    // Backends without a machine-mode fetch trace return an empty ring and
    // the panel stays blank - the classic UI never calls these.
    virtual std::vector<PcHistEntry> GetPcHistory(int cpu) {
        (void)cpu;
        return {};
    }
    // TSConf video/DMA register block (§5.2 board). Returns false when the
    // machine has no TSConf (the board band is not painted).
    virtual bool GetTsConf(TsConfState& out) {
        (void)out;
        return false;
    }
};
