...
MockBackend::MockBackend(std::unique_ptr<ITestSet> testSet) {
    ...
    tsConf_ = seed_.isTsConf;
    ts_ = seed_.ts;
    for (const PcHistEntry& e : seed_.pcHist) {
        if (pcRingCount_ >= 32) break;
        pcRing_[pcRingCount_++] = e;
    }
}
...
Z80EX_BYTE MockBackend::MemReadCb(Z80EX_CONTEXT*, Z80EX_WORD addr, int m1State, void* user) {
    MockBackend* self = static_cast<MockBackend*>(user);
    if (m1State != 0) self->RecordPcFetch(addr);  // fork: M1 fetch -> PC history
...
void MockBackend::RecordPcFetch(uint16_t addr) {
    const int keep = pcRingCount_ < 32 ? pcRingCount_ : 31;
    // shift down (newest stays at [0]); the ring cap is 32, the panel shows 28
    for (int i = keep; i > 0; --i) pcRing_[i] = pcRing_[i - 1];
    pcRing_[0] = PcHistEntry{ts_.page[(addr >> 14) & 3], addr};
    if (pcRingCount_ < 32) ++pcRingCount_;
}

std::vector<PcHistEntry> MockBackend::GetPcHistory(int) {
    return std::vector<PcHistEntry>(pcRing_.begin(), pcRing_.begin() + pcRingCount_);
}

bool MockBackend::GetTsConf(TsConfState& out) {
    if (!tsConf_) return false;
    out = ts_;
    return true;
}
