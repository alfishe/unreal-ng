...
TestSetState GoldenSampleState::Build() {
    ...
    st.machine.model = MemoryModelHint::TsConf;  // fork: 157-col TSConf variant
    st.machine.ayScheme = AyScheme::Single;
    st.machine.trdosPresent = true;
    st.machine.gsPresent = false;
    st.machine.gsDmaAddr = 0;
    st.machine.pages = {{
        PageInfo{"ROM 0", true},   // fork golden: window 0 maps ROM 0
    ...
    st.ui.activeWindow = 1;  // TRACE focused (0=REGS 1=TRACE 2=MEM 3=BANKS)
    st.ui.regsCurs = 0;
    ...
    // TSConf register block (unreal_dbg_render.py sample_state() "ts" dict).
    st.isTsConf = true;
    TsConfState& ts = st.ts;
    ts.vconf = 0x00;
    ts.rres = 0;
    ts.nogfx = false;
    ts.notsu = false;
    ...
    // PC history: 28 descending addresses below 0x800F; the page shown is
    // the page mapped at the fetch window ([0,5,2,0][addr >> 14]).
    st.pcHist.reserve(28);
    for (int i = 0; i < 28; ++i) {
        const uint16_t addr = static_cast<uint16_t>(0x800F - i);
        st.pcHist.push_back(PcHistEntry{ts.page[(addr >> 14) & 3], addr});
    }
