		...
		for(mpos=(dasm->mnemonic); *mpos && out_len < output_size; mpos++)
		{
			*stmp='\0';
			...
			switch(*mpos)
			{
				...
				case '$':
				case '%':
					/* poc018 patch: count the displacement byte only when it is
					 * actually read here. For DD/FD+CB the displacement was already
					 * consumed (and counted) above, so the original unconditional
					 * bytes++ returned length+1 for all DDCB/FDCB instructions. */
					if(!have_disp) { disp_u = readbyte_cb(addr++,user_data); bytes++; }
