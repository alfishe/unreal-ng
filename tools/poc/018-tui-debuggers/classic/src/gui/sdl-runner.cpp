// sdl-runner.cpp - SDL3 window runner implementation for classic debugger.
                ...
                if (event.type == SDL_EVENT_MOUSE_BUTTON_DOWN) {
                    if (event.button.button == SDL_BUTTON_LEFT) {
                        ...
                        if (curWinW > 0 && curWinH > 0) {
                            ...
                            if (HandleMouseClick(be, ui, cx, cy, false)) {
                    ...
                    if (event.wheel.y > 0) {
                        HandleArrowKey(be, ui, NavKey::Up, false);
                        if (ui.activeWidget == WidgetId::Trace) followCursor();
                        needsRepaint = true;
                    } else if (event.wheel.y < 0) {
                        HandleArrowKey(be, ui, NavKey::Down, false);
                        if (ui.activeWidget == WidgetId::Trace) followCursor();
                        needsRepaint = true;
                    }
                    ...
                    if (cmdOrCtrl && (isPlus || key == SDLK_EQUALS)) {
                        ...
                        if (shift) FocusPrevWidget(ui, false);
                        else FocusNextWidget(ui, false);
                        needsRepaint = true;
                    } else if (key == SDLK_UP) {
                        HandleArrowKey(be, ui, NavKey::Up, false);
                        if (ui.activeWidget == WidgetId::Trace) followCursor();
                        ...
                        HandleArrowKey(be, ui, NavKey::Left, false);
                        needsRepaint = true;
                    } else if (key == SDLK_RIGHT) {
                        HandleArrowKey(be, ui, NavKey::Right, false);
                        needsRepaint = true;
                    } else if (key == SDLK_PAGEUP) {
                        HandleArrowKey(be, ui, NavKey::PgUp, false);
                        if (ui.activeWidget == WidgetId::Trace) followCursor();
                        needsRepaint = true;
                    } else if (key == SDLK_PAGEDOWN) {
                        HandleArrowKey(be, ui, NavKey::PgDn, false);
                        if (ui.activeWidget == WidgetId::Trace) followCursor();
                        needsRepaint = true;
