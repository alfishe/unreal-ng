// paint.h - v0 frame painters over the golden §2 geometry.
//
// PaintClassicFrame fills the whole 80x30 grid from live backend data using
// the measured golden column layout:
...
// This is the runnable approximation: the panels milestone tightens each
// painter to golden-exactness (attributes, quirks, full code-page glyphs)
#pragma once
...
struct UiState {
    ...
    int activeWindow = 1;         // legacy: 0 = regs, 1 = trace, 2 = memory
    ...
    bool showBank = false;        // bank cursor visible
    std::string tsActive;         // visually selected board control name
...
// Individual painters (composed by PaintClassicFrame).
