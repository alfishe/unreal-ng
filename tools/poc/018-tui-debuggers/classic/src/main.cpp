// main.cpp - dbgclassic entry point: backend + FTXUI terminal loop.
//
// v0 command slice (the input milestone wires the full §9 key map):
//   F5/r run    F11/s step    F9/b toggle BPX at trace cursor
//   Up/Down move trace cursor    1/2/3 switch focused window
    ...
    body |= ftxui::CatchEvent([&](ftxui::Event e) {
        if (e.is_mouse()) {
            if (e.mouse().button == ftxui::Mouse::Left && e.mouse().motion == ftxui::Mouse::Pressed) {
                if (dbg::HandleMouseClick(*app.be, app.ui, e.mouse().x, e.mouse().y, false)) {
        ...
        if (e == ftxui::Event::Tab) {
            dbg::FocusNextWidget(app.ui, false);
            app.Repaint();
            return true;
        }
        if (e == ftxui::Event::TabReverse) {
            dbg::FocusPrevWidget(app.ui, false);
            app.Repaint();
            return true;
        }
        if (e == ftxui::Event::ArrowUp) {
            dbg::HandleArrowKey(*app.be, app.ui, dbg::NavKey::Up, false);
            if (app.ui.activeWidget == dbg::WidgetId::Trace) app.FollowCursor();
            app.Repaint();
            return true;
        }
        if (e == ftxui::Event::ArrowDown) {
            dbg::HandleArrowKey(*app.be, app.ui, dbg::NavKey::Down, false);
            if (app.ui.activeWidget == dbg::WidgetId::Trace) app.FollowCursor();
            app.Repaint();
            return true;
        }
        if (e == ftxui::Event::ArrowLeft) {
            dbg::HandleArrowKey(*app.be, app.ui, dbg::NavKey::Left, false);
            app.Repaint();
            return true;
        }
        if (e == ftxui::Event::ArrowRight) {
            dbg::HandleArrowKey(*app.be, app.ui, dbg::NavKey::Right, false);
            app.Repaint();
            return true;
        }
        if (e == ftxui::Event::PageUp) {
            dbg::HandleArrowKey(*app.be, app.ui, dbg::NavKey::PgUp, false);
            if (app.ui.activeWidget == dbg::WidgetId::Trace) app.FollowCursor();
            app.Repaint();
            return true;
        }
        if (e == ftxui::Event::PageDown) {
            dbg::HandleArrowKey(*app.be, app.ui, dbg::NavKey::PgDn, false);
            if (app.ui.activeWidget == dbg::WidgetId::Trace) app.FollowCursor();
            app.Repaint();
            return true;
        }
        ...
        if (e == ftxui::Event::F5 || (isChar && ch == "r" && !app.ui.isEditing)) {
