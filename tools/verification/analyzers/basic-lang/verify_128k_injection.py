#!/usr/bin/env python3
"""
128K BASIC Command Injection Verification

Tests that commands can be injected into the 128K BASIC editor and appear on screen.
Verifies injection only - does not execute commands.
"""
...
class Basic128KInjectionTest:
    ...
    def switch_to_128k_basic(self) -> bool:
        """Switch Pentagon to 128K BASIC mode using the /basic/mode API"""
        self.print_info("Switching to 128K BASIC mode...")
        
        try:
            url = f"{self.base_url}/api/v1/emulator/{self.emulator_uuid}/basic/mode"
            response = self.session.post(url, json={"mode": "128k"}, timeout=10)
            
            if response.status_code == 200:
                data = response.json()
                if data.get('success'):
                    self.print_success(f"Switched to 128K BASIC: {data.get('message')}")
                    time.sleep(2)
                    return True
                else:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if data.get('success'):
                    ...
                    # Wait for ROM to process keypress and refresh screen
                    self.print_info("Waiting for screen refresh...")
                    time.sleep(2.0)
                    return True
                else:
    ...
    def cleanup(self):
        """Delete the emulator instance"""
        if self.emulator_uuid:
            try:
                self.session.delete(f"{self.base_url}/api/v1/emulator/{self.emulator_uuid}")
            except:
                pass
    ...
    def run_test(self, command: str = "PRINT 42") -> bool:
        ...
        print("=" * 60)
        print("128K BASIC Command Injection Verification")
        print("=" * 60)
        ...
        try:
            ...
            # Switch to 128K BASIC mode
            self.print_step("3", "Switching to 128K BASIC mode...")
            if not self.switch_to_128k_basic():
                return False
            
            # Capture screen after mode switch
            self.print_step("4", "Capturing screen state...")
            success, lines = self.capture_screen_ocr("before injection")
            ...
            print("\n" + "=" * 60)
            print("✓ 128K INJECTION TEST PASSED")
            print("=" * 60)
            return True
