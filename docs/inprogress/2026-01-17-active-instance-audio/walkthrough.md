# Audio Binding on Tile Focus - Walkthrough

## Summary

Implemented audio binding that follows tile focus in the videowall. When a tile receives keyboard focus, its emulator's audio is bound to the shared sound manager and sound generation is enabled. When focus moves to another tile, the previous tile's audio is unbound and disabled.
...
signals:
    /// Emitted when this tile receives keyboard focus
...
void VideoWallWindow::addEmulatorTile()
{
    ...
    if (emulator)
    {
        ...
        // Connect tile focus signal for audio binding
...
```
[14/15] Building CXX object CMakeFiles/unreal-videowall.dir/src/emulator/soundmanager.cpp.o
[15/15] Linking CXX executable bin/unreal-videowall
```

## Manual Testing

1. Run `./cmake-build-debug/bin/unreal-videowall`
2. Create 2+ tiles with `Cmd+N`
3. Click tile 1 → audio plays from tile 1
4. Click tile 2 → audio switches to tile 2, tile 1 goes silent
5. Repeat to verify clean handoff
