...
```mermaid
graph TD
    ENTER(ENTER) --> CMD_CHECK{HAS\nA TYPE I\nCOMMAND BEEN\nRECEIVED}
    CMD_CHECK -- NO --> CMD_CHECK
    CMD_CHECK -- YES --> SET_BUSY[SET BUSY RESET CRC\nSEEK ERROR DRQ INTRQ]
    SET_BUSY --> IS_H{IS\nH = 1\n?}
    ...
    STEP_IN -- YES --> SET_DIR[SET\nDIRECTION]
    STEP_IN -- NO --> STEP_OUT{IS\nCOMMAND\nSTEP OUT\n?}
    
    STEP_OUT -- YES --> RESET_DIR[RESET\nDIRECTION]
    STEP_OUT -- NO --> STEP{IS\nCOMMAND\nSTEP\n?}
    ...
    DR_TO_DSR --> DOES_TR_EQ_DSR{DOES\nTR = DSR\n?}
    DOES_TR_EQ_DSR -- YES --> NO_STEP
    DOES_TR_EQ_DSR -- NO --> IS_DSR_GT_TR{IS\nDSR > TR\n?}
    ...
    subgraph " "
    C --> ISSUE_STEP_C
    ISSUE_STEP --> ISSUE_ONE_STEP[ISSUE\nONE STEP PULSE]
    ISSUE_ONE_STEP --> DELAY[DELAY ACCORDING\nTO R1 R0 FIELD]
    DELAY --> IS_COMMAND_STEP{IS\nCOMMAND\nA STEP, STEP IN\nOR STEP OUT\n?}
    IS_COMMAND_STEP -- YES --> D((D))
    IS_COMMAND_STEP -- NO --> UPDATE_TR
    
    UPDATE_TR --> IS_DIR_1{IS\nDIRECTION\n= 1\n?}
    ...
    SET_HLD_2 --> HAS_15MS{HAS\n15 MS\nEXPIRED\n?}
    HAS_15MS -- NO --> HAS_15MS
    HAS_15MS -- YES --> IS_HLT{IS\nHLT = 1\n?}
    IS_HLT -- NO --> IS_HLT
    IS_HLT -- YES --> HAVE_5_INDEX{HAVE\n5 INDEX\nHOLES\nPASSED\n?}
    HAVE_5_INDEX -- YES --> ERROR_EXIT[INTRQ RESET BUSY\nSET SEEK ERROR]
    HAVE_5_INDEX -- NO --> HAS_IDAM{HAS\nID AM BEEN\nDETECTED\n?}
    HAS_IDAM -- NO --> HAVE_5_INDEX
    HAS_IDAM -- YES --> DOES_TR_EQ_ID{DOES\nTR = TRACK\nADDRESS OF ID\nFIELD\n?}
    DOES_TR_EQ_ID -- NO --> HAVE_5_INDEX
    DOES_TR_EQ_ID -- YES --> IS_CRC_ERROR{IS\nTHERE A\nCRC ERROR\n?}
    IS_CRC_ERROR -- YES --> SET_CRC[SET\nCRC\nERROR]
    IS_CRC_ERROR -- NO --> RESET_CRC[RESET\nCRC]
    SET_CRC --> HAVE_5_INDEX
    RESET_CRC --> SUCCESS[INTRQ\nRESET BUSY]
    end
```
...
```mermaid
graph TD
    ENTER(ENTER) --> CMD_RX{IS\nTYPE II\nCOMMAND\nRECEIVED}
    CMD_RX -- NO --> CMD_RX
    CMD_RX -- YES --> INIT[SET BUSY RESET DRQ LOST\nDATA RECORD NOT FOUND &\nSTATUS BITS 5 & 6 INTRQ]
    INIT --> READY{IS\nDISK\nREADY\n?}
    READY -- NO --> NOT_READY[INTRQ\nRESET BUSY]
    READY -- YES --> SSO[COPY S FLAG TO\nSSO LINE (1795/7 ONLY)]
    SSO --> SET_HLD[SET HLD]
    SET_HLD --> IS_E{IS E = 1 ?}
    IS_E -- NO --> HLT_CHECK
    IS_E -- YES --> WAIT_15[HAS\n15 MS\nEXPIRED\n?]
    WAIT_15 -- NO --> WAIT_15
    WAIT_15 -- YES --> HLT_CHECK
    
    HLT_CHECK{IS HLT = 1 ?}
    HLT_CHECK -- NO --> HLT_CHECK
    HLT_CHECK -- YES --> CHECK_TG43{IS\nTR > 43\n?}
    CHECK_TG43 -- YES --> SET_TG43[SET\nTG43]
    ...
    WPRT_CHECK{IS\nWRITE\nPROTECT\nON\n?}
    WPRT_CHECK -- YES --> IS_WRITE{IS\nCOMMAND\nWRITE\n?}
    IS_WRITE -- YES --> ERR_EXIT[INTRQ RESET BUSY\nSET WRITE PROTECT]
    IS_WRITE -- NO --> SEARCH
    WPRT_CHECK -- NO --> SEARCH
    
    SEARCH[1] --> FIVE_INDEX{HAVE\n5 INDEX HOLES\nPASSED\n?}
    FIVE_INDEX -- YES --> RNF[INTRQ RESET BUSY\nSET RECORD NOT FOUND]
    FIVE_INDEX -- NO --> IDAM{HAS\nIDAM\nBEEN\nDETECTED\n?}
    IDAM -- NO --> FIVE_INDEX
    IDAM -- YES --> TR_CHECK{DOES\nTR = TRACK\nADDRESS OF ID\nFIELD\n?}
    TR_CHECK -- NO --> FIVE_INDEX
    TR_CHECK -- YES --> SR_CHECK{DOES\nSR = SECTOR\nADDRESS OF ID\nFIELD\n?}
    SR_CHECK -- NO --> FIVE_INDEX
    SR_CHECK -- YES --> SIDE_CHECK{DOES\nS = SIDE NO\nOF\nID FIELD\n?}
    SIDE_CHECK -- NO --> FIVE_INDEX
    SIDE_CHECK -- YES --> LOAD_LEN[BRING IN SECTOR LENGTH FIELD\nSTORE LENGTH IN INTERNAL\nREGISTER]
    LOAD_LEN --> CRC_CHECK{IS\nTHERE A\nCRC ERROR\n?}
    CRC_CHECK -- YES --> SET_CRC[SET CRC\nSTATUS ERROR]
    SET_CRC --> FIVE_INDEX
    CRC_CHECK -- NO --> RESET_CRC[RESET\nCRC]
    RESET_CRC --> CMD_TYPE{IS\nCOMMAND\nWRITE\n?}
...
```mermaid
graph TD
    READ_FLOW[2] --> DATA_AM{HAS\nDATA AM\nOCCURED\nIN TIME\n?}
    DATA_AM -- NO --> RNF[INTRQ RESET BUSY\nSET RECORD NOT FOUND]
    DATA_AM -- YES --> REC_TYPE[PUT RECORD TYPE IN\nSTATUS REG BIT 5]
    REC_TYPE --> BYTE_ASM{HAS\nFIRST BYTE\nBEEN ASSEMBLED\nIN DSR\n?}
    BYTE_ASM -- NO --> BYTE_ASM
    BYTE_ASM -- YES --> SET_DRQ[SET DRQ]
    SET_DRQ --> NEXT_BYTE{HAS\nNEXT BYTE\nBEEN ASSEMBLED\nIN DSR\n?}
    NEXT_BYTE -- NO --> NEXT_BYTE
    NEXT_BYTE -- YES --> READ_CHECK{HAS\nDR BEEN\nREAD BY\nCOMPUTER\n(DRQ=0)\n?}
    READ_CHECK -- NO --> LOST[SET DATA\nLOST]
    READ_CHECK -- YES --> ALL_BYTES{HAVE\nALL BYTES\nBEEN INPUTTED\n?}
    LOST --> ALL_BYTES
    ALL_BYTES -- NO --> NEXT_BYTE
    ALL_BYTES -- YES --> CRC_CHECK{CRC\nERROR\n?}
    CRC_CHECK -- YES --> SET_ERR[INTRQ RESET BUSY\nSET CRC ERROR]
    CRC_CHECK -- NO --> IS_M{IS\nM = 1\n?}
    IS_M -- NO --> SUCCESS[INTRQ RESET BUSY]
    IS_M -- YES --> INC_SEC[+1 TO\nSECTOR REG]
    INC_SEC --> RETURN[4]
```
...
graph TD
    ...
    GAP_DELAY2 --> CHECK_LOAD{HAS\nDR BEEN\nLOADED BY\nCOMPUTER\n(DRQ=0)\n?}
    CHECK_LOAD -- NO --> LOST_EXIT[INTRQ RESET BUSY\nSET LOST DATA]
    CHECK_LOAD -- YES --> CHECK_DDEN{DDEN\n= 0?\n}
    
    CHECK_DDEN -- NO --> FM_DELAY[DELAY 1 BYTE OF GAP]
    CHECK_DDEN -- YES --> MFM_DELAY[DELAY 1 BYTE OF GAP]
    
    FM_DELAY --> WG_ON_FM[TURN ON WG & WRITE\n6 BYTES OF ZEROS]
    MFM_DELAY --> WG_ON_MFM[TURN ON WG & WRITE\n12 BYTES OF ZEROS]
    
    WG_ON_FM --> WRITE_AM[WRITE DATA AM\nACCORDING TO A0 FIELD\nOF WRITE COMMAND]
    WG_ON_MFM --> WRITE_AM
    
    WRITE_AM --> DR_TO_DSR[DR TO DSR SET DRQ]
    DR_TO_DSR --> WRITE_BYTE[WRITE BYTE TO DISK]
    WRITE_BYTE --> CHECK_NEXT_LOAD{HAS\nDR BEEN\nLOADED\n(DRQ=0)\n?}
    CHECK_NEXT_LOAD -- NO --> SET_LOST[SET DATA\nLOST\nWRITE BYTE\nOF ZEROS]
    CHECK_NEXT_LOAD -- YES --> DONE_BYTES{HAVE\nALL BYTES\nBEEN WRITTEN\n?}
...
```mermaid
graph TD
    ENTER(ENTER) --> IS_WRITE{IS\nTHIS A\nWRITE TRACK\n?}
    IS_WRITE -- NO --> READ_ADDR_FLOW[READ_ADDR_FLOW]
    IS_WRITE -- YES --> INIT[SET BUSY RESET DRQ\nLOST DATA STATUS\nBITS 4 & 5]
    INIT --> READY{IS\nDISK\nREADY\n?}
    READY -- NO --> EXIT[INTRQ\nRESET BUSY]
    READY -- YES --> SSO[COPY S FLAG TO\nSSO LINE (1795/7 ONLY)]
    SSO --> SET_HLD[SET HLD]
    ...
    UPD_TG43 --> WPRT_CHECK{IS\nWPRT = 0\n?}
    WPRT_CHECK -- YES --> ERR_WPRT[INTRQ RESET\nBUSY SET WPRT]
    WPRT_CHECK -- NO --> SET_DRQ[SET DRQ]
    SET_DRQ --> DELAY3[DELAY 3 BYTE\nTIMES]
    DELAY3 --> CHECK_SERV{HAS\nDRQ BEEN\nSERVICED\n?}
    CHECK_SERV -- NO --> LOST[SET INTRQ\nLOST DATA\nRESET BUSY]
    
    CHECK_SERV -- YES --> INDEX_CHECK{HAS\nINDEX\nPULSE\nOCCURED\n?}
    ...
    CHECK_MFM -- NO --> CHECK_F7{DOES\nDSR = F7\n?}
    CHECK_F7 -- YES --> WR_CRC[WRITE 2 CRC\nCHARS CLR = FF]
    CHECK_F7 -- NO --> CHECK_FC{DOES\nDSR = FC\n?}
    CHECK_FC -- YES --> WR_FC[WRITE FC\nCLK = D7]
    CHECK_FC -- NO --> CHECK_MARKS{DOES\nDSR = FE, OR\nF8-FB\n?}
    CHECK_MARKS -- YES --> WR_MARK[WRITE FE OR\nF8-FB CLK = C7\nINITIALIZE CRC]
    CHECK_MARKS -- NO --> WR_DSR[WRITE DSR\nCLK = FF]
    ...
    INDEX_CHK_2 -- NO --> LOAD_CHK{HAS\nDR BEEN\nLOADED\n?}
    LOAD_CHK -- YES --> A
    LOAD_CHK -- NO --> WRITE_ZEROS[WRITE\nBYTE OF ZEROS\nSET DATA LOST]
    WRITE_ZEROS --> A
    end
    
    subgraph " "
    MFM_OP --> CHECK_F5{DOES\nDSR = F5\n?}
    CHECK_F5 -- YES --> WR_A1[WRITE 'A1' IN MFM\nWITH MISSING CLOCK\nINITIALIZE CRC]
    CHECK_F5 -- NO --> CHECK_F6{DOES\nDSR = F6\n?}
    CHECK_F6 -- YES --> WR_C2[WRITE 'C2' IN MFM\nWITH MISSING CLOCK]
    CHECK_F6 -- NO --> CHECK_F7_MFM{DOES\nDSR = F7\n?}
    CHECK_F7_MFM -- YES --> WR_CRC_MFM[WRITE 2 CRC\nCHARS]
    CHECK_F7_MFM -- NO --> WR_DSR_MFM[WRITE DSR\nIN MFM]
...
```mermaid
graph TD
    ENTER(ENTER) --> INIT[SET BUSY\nRESET STATUS\nBITS 2, 4, 5]
    INIT --> READY{READY\n?}
    READY -- NO --> ERR[INTRQ\nRESET BUSY]
    READY -- YES --> SSO[COPY S FLAG\nTO SSO LINE\n(1795/7 ONLY)]
    SSO --> SET_HLD[SET HLD]
    SET_HLD --> IS_E{E = 1\n?}
    ...
    HLT_CHECK{HLT = 1\n?}
    HLT_CHECK -- NO --> HLT_CHECK
    HLT_CHECK -- YES --> UPD_TG43[TG43\nUPDATE]
    UPD_TG43 --> IS_READ_TRK{READ\nTRACK\n?}
    IS_READ_TRK -- YES --> A((A))
    IS_READ_TRK -- NO --> B((B))
    ...
    A --> INDEX_1{INDEX\nPULSE\n?}
    INDEX_1 -- NO --> INDEX_1
    INDEX_1 -- YES --> SHIFT[SHIFT ONE BIT\nINTO DSR]
    SHIFT --> INDEX_2{INDEX\nPULSE\n?}
    INDEX_2 -- YES --> SUCCESS[SET INTRQ\nRESET BUSY]
    INDEX_2 -- NO --> AM_DET{ADDRESS\nMARK DETECTED\n?}
    AM_DET -- YES --> ASSEMBLY
    AM_DET -- NO --> CHECK_8{HAVE 8\nBITS BEEN\nASSEMBLED\n?}
    CHECK_8 -- NO --> SHIFT
    CHECK_8 -- YES --> ASSEMBLY
    
    ASSEMBLY[IS DR\nEMPTY\n?]
    ASSEMBLY -- NO --> SET_LOST[SET LOST\nDATA BIT]
    ASSEMBLY -- YES --> TRANSFER[TRANSFER\nDSR TO DR]
    SET_LOST --> TRANSFER
    TRANSFER --> SET_DRQ[SET\nDRQ]
    SET_DRQ --> SHIFT
    end
    
    subgraph Read_Address
    B --> INDEX_6{HAVE 6\nINDEX HOLES\nPASSED\n?}
    INDEX_6 -- YES --> RNF[RESET BUSY\nSET INTRQ\nSET RNF]
    INDEX_6 -- NO --> IDAM{HAS\nIDAM BEEN\nDETECTED\n?}
    IDAM -- NO --> INDEX_6
    IDAM -- YES --> SHIFT_BYTE[SHIFT 1 BYTE\nINTO DSR]
    SHIFT_BYTE --> TRANS_DR[TRANSFER\nBYTE TO DR]
    TRANS_DR --> SET_DRQ_RA[SET DRQ]
    SET_DRQ_RA --> HAVE_6{HAVE 6\nBYTES BEEN\nREAD\n?}
    HAVE_6 -- NO --> SHIFT_BYTE
    HAVE_6 -- YES --> STORE_TR[TRANSFER TRACK\nNUMBER TO SECTOR\nREGISTER]
    STORE_TR --> CRC_CHECK{CRC\nERROR\n?}
    CRC_CHECK -- YES --> SET_CRC[SET CRC\nERROR BIT]
    CRC_CHECK -- NO --> EXIT_RA
    SET_CRC --> EXIT_RA[SET INTRQ\nRESET BUSY]
    end
```
