...
| :------------------------------------ | :------------------------------------------------------------------- |:-------------------------------------------------------------------------|:----------------------------------------------------|:----------------------------------------------------|:---------------------------------------------------------------------|:-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------| :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **DRQ Service Timeout (Lost Data)**   | READ SECTOR, WRITE SECTOR, READ ADDRESS, READ TRACK, WRITE TRACK     | MFM: 32 µs <br/> FM: 64 µs                                               | MFM: 32 µs <br/> FM: 64 µs                          | MFM: 32 µs <br/> FM: 64 µs                          | MFM: ~112 T-states <br/> FM: ~224 T-states                           | **These absolute timeout values are solely determined by FDD transfer speed (one byte-time) and encoding (MFM/FM). Independent of FDC/CPU clocks.** MFM (250 Kbit/s = 32 µs/byte). FM (125 Kbit/s = 64 µs/byte). **Note:** MB8876A specifies DRQ service times of 11.5-13.5 µs (MFM) and 27.5-31.5 µs (FM), representing the latest safe response time within the byte window. | Lost Data (Status Register bit 2 or S2). CPU must R/W Data Register within this fixed **ONE FDD byte-time** of DRQ. The FDC uses its clock to measure if this external deadline is missed. |
...
*   MFM data rate: 1 byte = 32 µs (250 kbit/s ÷ 8 bits = 31.25 kbyte/s)
*   FM data rate: 1 byte = 64 µs (125 kbit/s ÷ 8 bits = 15.625 kbyte/s)
...
*   **DRQ Service Timeout (Lost Data):**
    *   The *requirement* (e.g., 32 µs for MFM) is dictated by the data rate from the disk (1 byte every 32 µs for MFM). The FDC and CPU *must* service DRQ within this window.
