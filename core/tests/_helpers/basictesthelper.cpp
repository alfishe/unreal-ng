...
BasicTestHelper::BasicTestHelper(Emulator* emulator)
    : _emulator(emulator)
{
}

bool BasicTestHelper::injectAndRun(const std::string& basicProgram)
{
    if (!_emulator) {
...
bool BasicTestHelper::injectAndRunViaKeyboard(const std::string& basicProgram, unsigned cyclesToRun)
{
...
void BasicTestHelper::typeRunCommand()
{
    EmulatorContext* context = _emulator->GetContext();
    if (!context || !context->pKeyboard) {
        std::cout << "[BasicTestHelper] ERROR: No keyboard available" << std::endl;
...
void BasicTestHelper::runCycles(unsigned cycles)
{
    if (!_emulator) return;
