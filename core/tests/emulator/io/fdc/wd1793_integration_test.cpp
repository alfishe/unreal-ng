...
TEST_F(WD1793_Integration_Test, TRDOS_FORMAT_FullOperation)
{
    ...
    // STEP 6b: Wait until FORMAT completes (A> prompt returns)
    ...
    while (!formatDone && std::chrono::steady_clock::now() < deadline)
    {
        ...
        // Check for format completion (A> prompt visible)
    ...
    ASSERT_TRUE(formatDone) << "FORMAT did not complete within 90 seconds";
