...
void app_destroy(app_emulator* emu)
{
    ...
    if (emu->emu)
    {
        EmulatorManager::GetInstance()->RemoveEmulator(emu->emu->GetId());
        emu->emu.reset();
    }
    delete emu;
}
...
app_result app_frame_info(app_emulator* emu, uint16_t* w, uint16_t* h, uint64_t* latch_ts_us)
{
    ...
    return emu->video.GetFrameInfo(emu->emu.get(), w, h, latch_ts_us);
}
...
app_result app_copy_frame(app_emulator* emu, void* dst_rgba8, size_t dst_size)
{
    ...
    return emu->video.CopyFrame(emu->emu.get(), dst_rgba8, dst_size);
}
...
void app_set_present_delay(app_emulator* emu, uint8_t frames)
{
    if (emu && emu->emu)
    {
        emu->video.SetPresentDelay(emu->emu.get(), frames);
    }
}
...
app_result app_audio_open_device(app_emulator* emu)
{
    ...
    return emu->audio.OpenDevice(emu->emu.get());
}
...
app_result app_audio_attach_pull(app_emulator* emu, uint32_t device_rate)
{
    ...
    return emu->audio.AttachPull(emu->emu.get(), device_rate);
}
...
{
    if (!emu || !emu->emu || !key_name || key_name[0] == '\0')
        return APP_ERR_ARG;
    ...
    std::string targetId = emu->emu->GetId();
