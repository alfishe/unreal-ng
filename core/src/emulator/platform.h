...
constexpr char const* NC_FDD_DISK_INSERTED = "FDD_DISK_INSERT";                 // Disk image inserted (payload: FDDDiskPayload)
...
constexpr char const* NC_AUDIO_ACTIVITY = "AUDIO_ACTIVITY";                     // Audio source activity changed (payload: AudioActivityPayload). Posted by SoundManager (AudioActivityIndicators): every frame while a source's LED was lit within the last second, once when that ends.
