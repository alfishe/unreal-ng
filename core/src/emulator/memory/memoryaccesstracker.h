...
///        for MemoryAccessTracker's counter arrays (up to 24 MiB each).
///
...
/// on the same machine — a ~290x difference that shows up as a flat multi-hundred-millisecond cost
/// in every test or profiling session that enables memory tracking (`AllocateCounters()` pays it
/// three times over, once per physical-memory counter array).
///
...
///
/// This class is deliberately minimal: it exposes only the operations `MemoryAccessTracker`
/// actually uses (subscript indexing, size/empty/data queries, pointer-based iteration, and
/// an explicit bulk `zero()` via `std::memset`).
///
