...
// Constructor
MemoryAccessTracker::MemoryAccessTracker(Memory* memory, EmulatorContext* context)
    : _memory(memory), _context(context)
{
...
bool MemoryAccessTracker::AddMonitoredPort(const std::string& name, uint16_t port, 
                                         const MonitoringOptions& options)
{
...
void MemoryAccessTracker::UpdateRegionStats(uint16_t address, uint8_t value, uint16_t callerAddress, AccessType accessType)
{
        ...
        for (size_t index : it->second)
        {
            ...
            if (region.mode == _currentMode)
            {
                ...
                if (region.options.trackDataFlow && 
                    ((static_cast<uint8_t>(accessType) & (static_cast<uint8_t>(AccessType::Read) | static_cast<uint8_t>(AccessType::Write))) != 0))
                {
...
void MemoryAccessTracker::AddToDataFlowMap(std::unordered_map<uint8_t, uint32_t>& map, uint8_t value, uint32_t maxEntries)
{
...
void MemoryAccessTracker::AddToCallerMap(std::unordered_map<uint16_t, uint32_t>& map, uint16_t callerAddress, uint32_t maxEntries)
{
...
uint32_t MemoryAccessTracker::GetZ80BankTotalAccessCount(uint8_t bank) const
{
    ...
    if (bankWasRead || bankWasWritten || bankWasExecuted)
    {
        ...
        {
            if (bankWasRead) result += _z80ReadCounters[addr];
            if (bankWasWritten) result += _z80WriteCounters[addr];
            if (bankWasExecuted) result += _z80ExecuteCounters[addr];
        }
    }
    ...
    for (const auto& region : _monitoredRegions)
    {
        if (region.mode == _currentMode)
        {
            ...
            if (region.options.trackCallers && !region.stats.callerAddresses.empty())
            {
                ...
                std::vector<std::pair<uint16_t, uint32_t>> sortedCallers(region.stats.callerAddresses.begin(), region.stats.callerAddresses.end());
                ...
                {
                    ss << "    0x" << std::hex << sortedCallers[i].first << std::dec 
                       << ": " << sortedCallers[i].second << " accesses\n";
                }
            }
            ...
            if (region.options.trackDataFlow && !region.stats.dataValues.empty())
            {
                ...
                std::vector<std::pair<uint8_t, uint32_t>> sortedValues(region.stats.dataValues.begin(), region.stats.dataValues.end());
...
bool MemoryAccessTracker::HasActivity(uint16_t start, uint16_t end) const
{
    ...
    for (uint32_t addr = start; addr <= end; addr++)
    {
        ...
        if (physOffset < _physReadCounters.size() && 
            (_physReadCounters[physOffset] > 0 || 
             _physWriteCounters[physOffset] > 0 || 
...
std::string MemoryAccessTracker::GenerateSegmentReport() const
{
    ...
    for (const auto& segment : _segments)
    {
        ...
        {
            case TrackingEvent::Frame: ss << "Frame"; break;
            case TrackingEvent::Interrupt: ss << "Interrupt"; break;
            case TrackingEvent::Custom: ss << "Custom"; break;
        }
        ss << "\n";
        ...
        if (!segment.regionStats.empty())
        {
            ...
            for (const auto& pair : segment.regionStats)
            {
                ...
                ss << "      Total Accesses: " << (pair.second.readCount + pair.second.writeCount + pair.second.executeCount) << "\n";
            }
        }
