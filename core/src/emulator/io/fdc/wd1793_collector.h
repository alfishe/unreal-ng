...
public:
        ...
        CommandRecord
            (
                ...
                const std::string& desc = "" // Human-readable description
                    ...
                    statusRegister(status), diskInserted(disk), currentTrack(curTrack),
    ...
    /// Collects command information for logging and analysis
    /// @param port Port number that was accessed
    /// @param value The value that was written to or read from the port
    /// @param isWrite True if this is a write operation, false if read
    void collectWD1793CallsInfo(WD1793& wd1793, uint8_t port, uint8_t value, bool isWrite);
