...
// Nominal track length in FM (single density) mode: 125 Kbps => 15625 bytes/s => 3125 bytes per 200 ms revolution
...
public:
    ...
    /// CRC-16/CCITT as generated by the FD179x in FM (single density) mode.
    /// The CRC generator is preset to 0xFFFF right before the address mark byte (no A1 A1 A1 preamble in FM),
    /// so the CRC covers the mark byte itself and everything after it.
    /// @param ptr Pointer to the address mark byte
    /// @param size Number of bytes (mark + fields)
    /// @return CRC with bytes swapped to on-disk order (high byte first in memory)
    static uint16_t crcWD1793FM(uint8_t* ptr, uint16_t size)
    {
        uint16_t crc = 0xFFFF;
    ...
    /// Used for UDI disk format (Ultra Disk Image by Alex Makeev)
    /// @details CRC-32 (polynomial 0xEDB88320) with one deliberate quirk kept for compatibility with every UDI
    /// writer in existence: the accumulator is a signed int, so `crc >>= 1` is an arithmetic shift.
    /// The caller starts with crc = -1 and feeds the whole file except the trailing 4 CRC bytes.
    /// Verified against testdata/loaders/udi/*.udi.
    static int crcUDI(int &crc, const unsigned char *buf, size_t len)
    {
        while (len--)
        {
            crc ^= -1 ^ *buf++;
