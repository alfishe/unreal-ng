...
bool BasicEncoder::injectIntoMemory(Memory* memory, const std::vector<uint8_t>& tokenizedProgram, uint16_t progStart)
{
...
bool BasicEncoder::isTRDOSInitialized(Memory* memory)
{
    ...
    uint16_t chansValue = memory->DirectReadFromZ80Memory(CHANS) | (memory->DirectReadFromZ80Memory(CHANS + 1) << 8);
...
bool BasicEncoder::stackContainsDOSReturnAddress(Memory* memory, uint16_t z80SP, int maxDepth)
{
    ...
    for (int i = 0; i < maxDepth; i++)
    {
        ...
        uint16_t retAddr =
            memory->DirectReadFromZ80Memory(stackAddr) | (memory->DirectReadFromZ80Memory(stackAddr + 1) << 8);
...
BasicEncoder::InjectionResult BasicEncoder::injectTo48K(Memory* memory, const std::string& command)
{
    ...
    uint16_t eLineAddr = memory->DirectReadFromZ80Memory(E_LINE) | (memory->DirectReadFromZ80Memory(E_LINE + 1) << 8);
...
BasicEncoder::InjectionResult BasicEncoder::injectCommand(Emulator* emulator, const std::string& command)
{
    ...
    // This avoids the move constructor that triggers GCC 13 false positive -Warray-bounds
    ...
    switch (state)
    {
        ...
        case BasicState::Menu128K:
            return {false, state, "Error: On 128K menu. Use autoNavigateAndInject() or enter BASIC first."};
