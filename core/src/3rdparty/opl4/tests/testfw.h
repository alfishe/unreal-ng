...
// FM channel 0 key-on (block 4, fn 771 ~ A4) — steady activity. Classic
// YMF262 register map: ch0 modulator = 0x20/0x40/0x60/0x80,
// carrier = +3 (0x23/0x43/0x63/0x83); routing C0 = CHA|CHB (bits include).
inline void KeyOnFmCh0(Opl4& c, uint64_t t)
{
    c.WriteFm(t, 0, 0x20, 0x01); // modulator: mult 1
    c.WriteFm(t, 0, 0x23, 0x01); // carrier: mult 1
...
int RunVectorTests();
void RunFmEngineTests();          // opl4fmtests.cpp (FM engine behaviour)
