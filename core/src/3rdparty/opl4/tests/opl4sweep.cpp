...
// snapshots.
//
// Register-map note: the engine uses the canonical YMF262 operator map
...
// side). FmOpAddr()/kRoute* carry the conventions.
#include "testfw.h"
...
// Minimum steady RMS of a TL0 unity-mix carrier (~23170 sine RMS at int
// scale, ~0.177 normalized by kNormScale).
constexpr double kFmCarrierRmsMin = 16000.0 * kNormScale;
...
// NEW armed so OPL3 register semantics apply.
...
// Map-agnostic invariants; the folded table (ymfm/silicon
...
void FmKslSweep()
{
    ...
    // Exact folded table (ymfm/silicon consumption semantics): at
...
// the final sum ring loud, cascade-only algorithms stay near-silent (the
// YMF262 algorithms 8-11).
// ---------------------------------------------------------------------------
void Fm4OpConnections()
{
    ...
    // Algorithms 8-11 (YMF262 silicon, as ymfm models them): the master C0
...
// releases (RR 15). While rhythm is enabled, B0 key-ons on ch7 are claimed
// by the percussion map and stay silent (YMF262 datasheet).
// ---------------------------------------------------------------------------
void FmRhythmSweep()
{
    ...
    // the voice only sounds once its 0xBD bit (HH, bit 0) keys it, per the
    // YMF262 datasheet (ymfm keys it from B0 too; unconfirmed on silicon).
    {
        TestChip tc;
        tc.chip.WriteWave(0, 0xF8, 0x00);
        tc.chip.WriteFm(0, 1, 0x05, 0x01);
...
// zero-crossing density relative to the sine. The shapes are the YMF262
// silicon table (as in ymfm).
// ---------------------------------------------------------------------------
void FmWaveformSweep()
{
    ...
    // Canonical YMF262 shapes (the silicon table, as in ymfm):
...
// ---------------------------------------------------------------------------
// FM output routing: C0 bits 4-7 steer the outputs over the full OPL4
// quad: CHA/CHB (bits 4/5) enable pair-A sides, CHC/CHD (bits 6/7) the
// second pair, summed into the same L/R mix — a side is audible when any
// of its enable bits is set.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Envelope rate cadence: the OPL family rate machinery halves the attack
// interval per rate index (shift ladder + sub-table rows), so AR
// half-level times must step ~2x per rate.
// ---------------------------------------------------------------------------
...
// register scales only its own block.
// ---------------------------------------------------------------------------
void MixFieldMatrix()
{
